  ...
  if (error) {
    return (
      <div style={{ width: size, height: size }}
        className="rounded-full bg-nsr-wine flex items-center justify-center border-2 border-white/25 flex-shrink-0">
...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      ...
      {/* ── Sidebar desktop ──────────────────────────────────────────── */}
      <aside
        className={`hidden md:flex flex-col flex-shrink-0 bg-[#002147] transition-all duration-200 ${open ? 'w-56' : 'w-[68px]'}`}
        style={{ position: 'sticky', top: 0, height: '100vh', overflowY: 'auto' }}
      >

        {/* Logo + toggle — misma fila cuando abierto, apilados cuando cerrado */}
        {open ? (
          <div className="flex items-center gap-3 h-16 px-4 border-b border-white/10 flex-shrink-0">
            ...
            <button onClick={() => setOpen(false)} title="Colapsar sidebar"
              className="w-7 h-7 flex items-center justify-center rounded-lg text-white/35 hover:text-white hover:bg-white/10 transition-colors flex-shrink-0">
              <span className="w-3.5 h-3.5"><IconChevronLeft /></span>
            </button>
          </div>
        ) : (
          <div className="flex flex-col items-center pt-3 pb-2 border-b border-white/10 flex-shrink-0 gap-2">
            <LogoNSR size={32} />
            <button onClick={() => setOpen(true)} title="Expandir sidebar"
              className="w-7 h-7 flex items-center justify-center rounded-lg text-white/35 hover:text-white hover:bg-white/10 transition-colors">
              <span className="w-3.5 h-3.5"><IconChevronRight /></span>
            </button>
          </div>
        )}
        ...
        <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
          {visible.map(item => (
            <NavLink key={item.to} to={item.to}
              ...
              className={({ isActive }) =>
                `flex items-center rounded-xl transition-colors ${open ? 'gap-3 px-3 py-2.5' : 'justify-center p-3'} ${
                  isActive
